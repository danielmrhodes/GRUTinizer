...
bool GCanvas::Process1DKeyboardPress(Event_t *event,UInt_t *keysym) {
  ...
  case kKey_e:
      ...
      
    {
      double x1, x2;
      if(fMarkers.at(fMarkers.size()-1)->localx < fMarkers.at(fMarkers.size()-2)->localx) {
	x1=fMarkers.at(fMarkers.size()-1)->localx;
	x2=fMarkers.at(fMarkers.size()-2)->localx;
      }
      else {
	x1=fMarkers.at(fMarkers.size()-2)->localx;
	x2=fMarkers.at(fMarkers.size()-1)->localx;
      }
  ...
  case kKey_C:
    {
      ...
      GetContextMenu()->Action((GH2*)hists.back(),((GH2*)hists.back())->Class()->GetMethodAny("RebinY"));
